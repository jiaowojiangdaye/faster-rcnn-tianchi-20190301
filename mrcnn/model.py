...
#                                              train_bn=config.TRAIN_BN)
    ...
    def unmold_bbox(self, detections, original_image_shape,
                          image_shape, window):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
...
#            if full_masks else np.empty(original_image_shape[:2] + (0,))
    ...
    def unmold_rpn(self, rois, class_ids, bboxes, original_image_shape,
                          image_shape, window):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
        ...
        # Extract boxes, class_ids, scores, and class-specific masks
        boxes = bboxes[:N, :4]
        class_ids = class_ids[:N, 4].astype(np.int32)
        scores = detections[:N, 5]
#        masks = mrcnn_mask[np.arange(N), :, :, class_ids]
...
#            if full_masks else np.empty(original_image_shape[:2] + (0,))
    ...
    def detect(self, images, verbose=0, given_rois=None):
        ...
        if self.mode == 'inference':
            detections, mrcnn_class, mrcnn_bbox, rpn_rois, rpn_class, rpn_bbox, rmac, feats_norm =\
        ...
        if self.mode == 'inference_rpn':
            ...
            for i, image in enumerate(images):
                final_rois, final_class_ids, final_scores =\
                    self.unmold_rpn(rpn_rois[i], rpn_class, rpn_bbox,
                                           image.shape, molded_images[i].shape,
                                           windows[i])
                results.append({
                    "rois": final_rois,
